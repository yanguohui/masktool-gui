...
- **检测灵敏度（smart / aggressive 生效）**：可自主调节智能识别的“宽严”，
...
## 检测灵敏度（针对 smart / aggressive）

智能模式（smart / aggressive）会用 jieba NER 自动识别人名、机构、地名等。
这些识别的**置信度偏低、容易误伤普通词语**（例如把"新建""中华民族""系统安全"
当成敏感信息脱敏）。因此工具额外提供了「检测灵敏度」开关，**只调节 NER 这一层**：

| 档位 | 行为 | 适用场景 |
|------|------|----------|
| `高（激进召回）` | NER 低置信项也参与脱敏，误伤最多 | 宁可错杀、事后人工复核 |
| `中（推荐）` | 手机/身份证/邮箱/金额/IP/MAC 等高置信信息照常脱敏，仅压制低置信 NER 误报 | 日常办公默认 |
| `低（保守精准）` | 只脱敏 ≥0.85 的 NER 项，几乎不误伤 | 对误报敏感 |
| `极低（审慎）` | 几乎只保留用户词库匹配项 | 误报零容忍、宁可漏掉 |

> 无论灵敏度如何，**手机、身份证、邮箱、金额、IP、MAC、日期**等高置信正则
> 识别，以及**用户词库**匹配，都**始终脱敏**，不会因为调低灵敏度而漏掉它们。
> 灵敏度仅影响 NER（人名/机构/地名等智能识别）的宽严。把短人名等交给
> 「用户词库」或 `strict` 模式更稳妥。
>
